...
Развёртывание simple-http-file-server с Nginx reverse proxy и автоматическими TLS-сертификатами через Let's Encrypt.

## Архитектура

```
Internet → :80  (HTTP)  → Nginx → 301 redirect → :443
Internet → :443 (HTTPS) → Nginx (TLS termination) → app:8080 (HTTP) → Go server
```

- Go-сервер работает на внутреннем порту (HTTP) внутри Docker-сети
...
cp .env.example .env
# отредактировать: DOMAIN, CERTBOT_EMAIL

# 2. Положить cookies.txt для Instagram
